...
---@type string[]
local help_lines = {
  "OneBeer Keymap Cheatsheet",
  "-------------------------",
  "Leader a   -> Sidekick",
  "Leader b   -> Buffers",
  "Leader c   -> Code actions (Fzf + LSP)",
  "Leader d   -> Debugger (nvim-dap)",
  "Leader f   -> Mini.files explorer",
  "Leader g   -> Grapple marks",
  "Leader jg  -> JSON to Go structs",
  "Leader jt  -> JSON to TypeScript types",
  "Leader l   -> Gitsigns hunk actions",
  "Leader m   -> Move lines",
  "Leader p   -> Precognition hints",
  "Leader q   -> Trouble + persistence sessions",
  "Leader s   -> Search suite (fzf-lua)",
  "Leader sd  -> Snacks dashboard",
  "Leader sv  -> Git pickers",
  "Leader u*  -> UI toggles (notifications, logs, virtual text, scroll)",
  "Leader uf  -> Format on save (buffer)",
  "Leader uF  -> Format on save (global)",
  "Leader uB  -> Lint toggle (buffer)",
  "Leader uL  -> Lint toggle (global)",
  "Leader v*  -> VCS helpers (gitbrowse, blame)",
  "Leader x   -> Multicursor remove",
  "[d / ]d    -> Prev/Next diagnostic",
  "[r / ]r    -> Navigate highlighted references",
