...
local function register(raw)
  ...
  for _, dep in ipairs(as_list(raw.dependencies)) do
    local dep_name = type(dep) == "string" and register({ dep }) or register(dep)
...
local function complete_pack(arg_lead, cmd_line)
  ...
  if #args <= 2 then
    return complete_matches({ "update" }, arg_lead)
  end
...
local function run_pack_command(ctx)
  ...
  vim.notify("Usage: :Pack[!] update [plugin ...]", vim.log.levels.ERROR, { title = "Pack" })
end
